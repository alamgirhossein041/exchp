...
def get_token_info(cm_futures_client: CMFutures, token_list: str) -> str:
    ...
    for token in token_list:
        for ele in cm_futures_client.ticker_24hr_price_change():
            if ele.get("symbol") == token + "USD_PERP":
                bold_pair_str = "*" + ele.get("pair") + "*"
                change24h_str = get_change_percent(
                    "change24h", float(ele.get("priceChangePercent")))
                tmp = "{}\nlast={}\n{}\n\n".format(bold_pair_str,
                                                   ele.get("lastPrice"),
                                                   change24h_str)
                res += tmp
